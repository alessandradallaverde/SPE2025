...
class RingSimulation(Simulation):
    ...
    # method to start an election; starting conditions: coordinator crashed
    # and n initiators
    def start_election(self):
        ...
        # the coordinator crashes (the one with the higher ID)
        ...
        for id in initiators:   # start election
            ...
            initiator.initiate()
            # send election to the initiator
            election_msg = ElectionRingMsg(initiator.id,-1,[])      
            yield initiator.queue.put(election_msg)
         
        # wait for finish_event to be triggered (election completed)
        yield self.finish_event     

        self.sim_stats.add_runtime(self.env.now)    # store in stats

        # stop all processes
        raise simpy.core.StopSimulation("Election finished")    
